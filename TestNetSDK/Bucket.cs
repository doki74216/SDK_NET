...
namespace TestNetSDK
{
    public class Bucket
    {
        public static void bucketSerial(String filePath, String bucketname1, String bucketname2)
        {
            System.Console.WriteLine("\nBucketSerial testing!!");
            ...
            //Brequest.WithBucketRegionName(bucketRegionName);
            ...
            s3Client.PutBucket(new PutBucketRequest().WithBucketName(bucketName2).WithUseClientRegion(true).WithGrants(new S3Grant().WithGrantee(new S3Grantee().WithCanonicalUser("canonicalidhrchu", "hrchu")).WithPermission(S3Permission.FULL_CONTROL)));
            GetBucketLocationResponse Locresult = s3Client.GetBucketLocation(new GetBucketLocationRequest().WithBucketName(bucketName));

            //GetService
            ListBucketsResponse result = s3Client.ListBuckets();
            //System.Console.WriteLine("Get Service Result:\n {0}\n", result.ResponseXml);
            ListBucketsResponse result2 = s3Client.ListBuckets(new ListBucketsRequest());
            //********************************************************************************************************************************
            //PutObject-apple.jpg
