...
namespace TestNetSDK
{
    class MPUmd5
    {
        public static void mpuSerial(String fPath)
        {
            ...
            //Upload Part * 2
            UploadPartResponse part1Result = s3Client.UploadPart(new UploadPartRequest().WithBucketName(bucketName).WithKey(objectNameA).WithPartNumber(1).WithUploadId(objectAUID).WithFilePath(fPath).WithFilePosition(10));
